...
export async function issuePowChallenge(): Promise<{
  ...
  const payload: PowChallengePayload = {
    ...
    exp: Date.now() + env.powChallengeTtlMs,
  };

  const payloadB64 = btoa(JSON.stringify(payload));
  const signature = await hmacSha256Base64(payloadB64, env.powSecret);
...
export async function verifyPowSolution(
  ...
  const expectedSignature = await hmacSha256Base64(payloadB64, env.powSecret);
